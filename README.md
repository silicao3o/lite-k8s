...
### Claude Code 연동
- **CLI 호출 클라이언트**: Claude Code CLI를 통한 AI 분석 요청
- **이상 컨텍스트 전달**: 컨테이너 정보, 메트릭, 로그를 AI에게 전달
- **응답 파싱**: JSON/텍스트 형식의 AI 응답 파싱
- **AI 판단 결과 변환**: AI 응답을 Playbook Action으로 변환
- **AI 자유 판단 모드**: Playbook 미등록 시 AI가 직접 판단
...
| Claude Code 연동 | CLI 호출, 컨텍스트 빌더, 응답 파싱, AI 자유 판단 | 23 |
| 로그 수집 | 로그 스트리밍, 로그 보존, 자동 정리 | 14 |
